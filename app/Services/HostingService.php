...
class HostingService
{
    ...
    public function suspend(HostingAccount $account, ?string $reason = null): void
    {
        ...
        $this->mutate($account, 'hosting.suspended', "Hosting account #{$account->id} suspended", [
    ...
    public function unsuspend(HostingAccount $account): void
    {
        ...
        $this->mutate($account, 'hosting.unsuspended', "Hosting account #{$account->id} reactivated", [
    ...
    public function changePackage(HostingAccount $account, int $newProductId): void
    {
        ...
        $this->mutate($account, 'hosting.package_changed', "Hosting account #{$account->id} package changed", [
    ...
    public function terminate(HostingAccount $account, ?string $reason = null): void
    {
        if ($account->status === self::STATUS_TERMINATED) {
            throw new RuntimeException("Hosting account #{$account->id} is already terminated.");
        }

        $this->mutate($account, 'hosting.terminated', "Hosting account #{$account->id} terminated", [
    ...
    private function assertStatus(HostingAccount $account, array $allowed, string $actionLabel): void
    {
        if (! in_array($account->status, $allowed, true)) {
            throw new RuntimeException(
                "Hosting account #{$account->id} cannot be {$actionLabel} from status '{$account->status}'."
            );
        }
    }
