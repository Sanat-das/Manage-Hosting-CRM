...
class DomainService
{
    ...
    /** @var array<string, string> status value => display label, for filter dropdowns and status selects. */
    ...
    /**
     * Registration price for a domain across a term in years.
     *
     * Uses the explicit `domain_pricing_terms` row when the TLD defines one
     * for the year count, otherwise the per-year register price (`domain_pricing`
     * table, falling back to the deterministic default) times the years.
     * Returns null for a malformed domain.
     */
    public function priceForTerm(string $domain, int $years): ?float
    {
        $years = max(1, min(10, $years));
        [$label, $tld] = $this->splitDomain(strtolower(trim($domain)));
        ...
        if ($pricing !== null) {
            $term = $pricing->terms()->where('term_years', $years)->first();

            if ($term?->register_price !== null) {
                return (float) $term->register_price;
            }

            if ($pricing->register_price !== null) {
                return (float) $pricing->register_price * $years;
            }
        }

        return (float) (self::DEFAULT_TLD_PRICES[$tld] ?? 999.0) * $years;
    }

    /**
     * Annual renewal price for a domain — the recurring amount snapshotted
     * onto the Domain record at registration time.
     */
    public function annualRenewPrice(string $domain): ?float
    {
        [, $tld] = $this->splitDomain(strtolower(trim($domain)));
        ...
        if ($pricing !== null) {
            return (float) ($pricing->renew_price ?? $pricing->register_price ?? (self::DEFAULT_TLD_PRICES[$tld] ?? 999.0));
        }

        return (float) (self::DEFAULT_TLD_PRICES[$tld] ?? 999.0);
    }

    /**
     * Sellable registration terms for a domain: the explicit
     * `domain_pricing_terms` rows when the TLD defines them, otherwise years
     * 1-10 at the per-year register price.
     *
     * @return list<array{years: int, price: float}>
     */
    public function termsFor(string $domain): array
    {
        [, $tld] = $this->splitDomain(strtolower(trim($domain)));
        ...
        $terms = $pricing?->terms()->orderBy('term_years')->get(['term_years', 'register_price']);

        if ($terms !== null && $terms->isNotEmpty()) {
            return $terms
                ->map(fn ($term) => [
                    'years' => (int) $term->term_years,
                    'price' => (float) $term->register_price,
                ])
                ->all();
        }

        return array_map(
            fn (int $years) => ['years' => $years, 'price' => $this->priceForTerm($domain, $years)],
            range(1, 10)
        );
    }
