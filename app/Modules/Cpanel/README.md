...
Implements `App\Contracts\Integrations\Capabilities\ProvisioningModule`, so
`OrderService::advanceAfterPayment()` routes to it through
`App\Services\Provisioning\ProvisioningDispatcher`.

## Setup

1. **Add the WHM server** — Admin → Servers. Set `server_type` to `cpanel` and
   fill in:
...
2. **Group the servers** — Admin → Server Groups. Put the server in a group and
   set each member's `priority` (lower is preferred).
3. **Point the product at it** — on the product, set *Provisioning module* to
   `cpanel` (cPanel/WHM) and the *Server group* to that group. Then enable this
   module on the product and set its config:
...
`panel_accounts` is a core table shared by all four panel drivers, with a
`panel` discriminator — the row records something living on someone else's
server, so it has to outlive any change to the integration code.
