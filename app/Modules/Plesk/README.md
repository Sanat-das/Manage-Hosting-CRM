...
`App\Contracts\Integrations\AbstractPanelModule`.
...
1. Add the server (Admin → Servers), `server_type` = `plesk`:
...
2. Put the server in a server group and point the product at it, with
   *Provisioning module* = `plesk`.
3. Module config on the product: `plan` (service plan name — blank uses Plesk's
