    ...
    <x-adminlte-card icon="bi bi-info-circle" title="Details">
        <table class="table table-sm table-borderless mb-0">
            <tbody>
                ...
                <tr><th class="text-muted">Premium</th><td><span class="badge {{ $pricing->premium ? 'bg-warning' : 'bg-secondary' }}">{{ $pricing->premium ? 'Yes' : 'No' }}</span></td></tr>
                <tr><th class="text-muted">Enabled</th><td><span class="badge {{ $pricing->enabled ? 'bg-success' : 'bg-secondary' }}">{{ $pricing->enabled ? 'Yes' : 'No' }}</span></td></tr>
