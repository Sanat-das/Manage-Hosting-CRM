...
@section('content')
    ...
    <x-adminlte-card icon="bi bi-info-circle" title="Details">
        <table class="table table-sm table-borderless mb-0">
            <tbody>
                <tr><th class="text-muted w-25">From</th><td><a href="{{ route('admin.products.show', $path->fromProduct) }}">{{ $path->fromProduct->name }}</a> <span class="badge bg-info">{{ $path->fromProduct->group?->name ?? '' }}</span></td></tr>
                <tr><th class="text-muted">To</th><td><a href="{{ route('admin.products.show', $path->toProduct) }}">{{ $path->toProduct->name }}</a> <span class="badge bg-info">{{ $path->toProduct->group?->name ?? '' }}</span></td></tr>
                <tr><th class="text-muted">Enabled</th><td>
                    @if ($path->enabled) <span class="badge bg-success">Enabled</span> @else <span class="badge bg-secondary">Disabled</span> @endif
                </td></tr>
            </tbody>
        </table>
    </x-adminlte-card>
