            ...
            @can('invoices.edit')
                @if (! in_array($invoice->status, [\App\Models\Invoice::STATUS_PAID, \App\Models\Invoice::STATUS_VOID, \App\Models\Invoice::STATUS_CANCELLED], true))
                    <form method="POST" action="{{ route('admin.invoices.send', $invoice) }}" class="d-inline">
                        @csrf
                        <button type="submit" class="btn btn-sm btn-outline-primary"><i class="bi bi-envelope me-1"></i> Send Invoice</button>
                    </form>
                @endif
            @endcan
            @can('payments.create')
                @if (! in_array($invoice->status, [\App\Models\Invoice::STATUS_PAID, \App\Models\Invoice::STATUS_VOID, \App\Models\Invoice::STATUS_CANCELLED], true))
                    <button type="button" class="btn btn-sm btn-outline-success" data-bs-toggle="collapse" data-bs-target="#recordPaymentForm" aria-expanded="false" aria-controls="recordPaymentForm"><i class="bi bi-cash-coin me-1"></i> Record Payment</button>
                @endif
            @endcan
        </div>
    </div>
        ...
        <div class="col-lg-4">
            <x-adminlte-card title="Summary">
                <table class="table table-sm table-borderless mb-0">
                    <tbody>
                        <tr><th class="text-muted">Customer</th><td>@if ($invoice->customer)<a href="{{ route('admin.customers.show', $invoice->customer) }}">{{ $invoice->customer->full_name }}</a>@else—@endif</td></tr>
