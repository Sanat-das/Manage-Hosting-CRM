    ...
    >
                        ...
                        @forelse ($products as $product)
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="product_ids[]"
                                       ...
                                       @checked(in_array((string) $product->id, array_map('strval', (array) old('product_ids', $productIds)), true))>
            ...
            <div class="col-md-6">
                <x-adminlte-select name="type" label="Input type">
                    @foreach ($typeOptions as $value => $label)
                        <option value="{{ $value }}" @selected(old('type', $productOption->type) === $value)>{{ $label }}</option>
                    @endforeach
                </x-adminlte-select>
            </div>
            <div class="col-md-6">
        ...
        <x-adminlte-card title="Option values" icon="bi bi-list-check" class="mt-3" body-class="p-3">
            <p class="text-muted small">
                ...
                Saving replaces the value list wholesale.
            </p>
