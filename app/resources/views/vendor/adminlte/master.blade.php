...
<head>
    ...
    {{-- No-flash color mode: apply the stored theme before any CSS paints.
         Dark is the default for first-time visitors. Mirrors the admin-lte
         ColorMode module's storage contract (localStorage['lte-theme'],
         values 'light'|'dark'|'auto') so the toggle keeps working unchanged. --}}
    <script>
        (function () {
            try {
                var stored = localStorage.getItem('lte-theme');
                var theme = stored && ['light', 'dark', 'auto'].includes(stored) ? stored : 'dark';
