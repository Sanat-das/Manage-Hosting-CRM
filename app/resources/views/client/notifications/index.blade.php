    ...
    <x-adminlte-card icon="bi bi-bell" title="Your Notifications">
        @if ($notifications->isNotEmpty() && auth()->user()->unreadNotifications()->count() > 0)
            <div class="text-end mb-3">
                <form method="POST" action="{{ route('client.notifications.markAllRead') }}" class="d-inline">
                    @csrf
                ...
                <tbody>
                    @forelse ($notifications as $notification)
                        <tr>
                            ...
                            <td class="text-end">
                                @if (! $notification->read_at)
                                    <form method="POST" action="{{ route('client.notifications.markRead', $notification) }}" class="d-inline">
                                        @csrf
