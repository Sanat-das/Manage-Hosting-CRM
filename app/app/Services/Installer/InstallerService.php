...
class InstallerService
{
    ...
    /**
     * Render the current migration status (ran vs pending) as plain text.
     *
     * Used by the controller to surface diagnostic output in the wizard when
     * the migrate step fails, so the failing migration is visible on the
     * form instead of requiring shell access to the server.
     */
    public function migrationStatus(): string
    {
        try {
            ...
            Artisan::call('migrate:status', ['--ansi' => false], $output);

            return trim($output->fetch());
        } catch (\Throwable $e) {
            return 'Could not determine migration status: '.$e->getMessage();
        }
    }
