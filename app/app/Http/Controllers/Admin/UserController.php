...
class UserController extends Controller
{
    ...
    /**
     * Reset a staff account password. Admin-invoked: bcrypt into password_hash,
     * never requires the current password (that is the change-password flow).
     */
    public function resetPassword(Request $request, User $user): RedirectResponse
    {
        ...
        $this->logActivity($user, 'password_reset', "Password reset for {$user->full_name}", [
            'user_id' => $user->id,
            'by' => auth()->user()?->email,
        ]);

        return back()->with('success', "Password for {$user->full_name} has been reset.");
    }
