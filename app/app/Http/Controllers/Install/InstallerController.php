...
class InstallerController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            // Append the migrate:status table so the first pending migration
            // (the one that failed) is visible right in the form instead of
            // requiring shell access to the server.
            $message .= "\n\nMigration status:\n".$this->installer->migrationStatus();

            return back()
                ->withInput()
                ->withErrors(['installer' => $message]);
        }
