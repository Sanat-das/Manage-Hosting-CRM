...
class ProductUpgradePathController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $query = ProductUpgradePath::with(['fromProduct', 'toProduct']);

        if ($search !== '') {
            $query->where(function ($q) use ($search) {
                $q->whereHas('fromProduct', fn ($p) => $p->where('name', 'like', "%{$search}%"))
                    ->orWhereHas('toProduct', fn ($p) => $p->where('name', 'like', "%{$search}%"));
            });
        }
        if ($status !== '') {
        ...
        return view('admin.product_upgrades.index', compact('paths', 'search', 'status'));
    }
