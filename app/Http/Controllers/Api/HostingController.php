...
class HostingController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $accounts = HostingAccount::query()
            ->with(['customer.user:id,email,first_name,last_name', 'product:id,name,type,price', 'server:id,name,ip_address'])
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $this->hostingService->audit(
            $account,
            'hosting.created',
            "Hosting account {$account->username} created via API",
    ...
    public function update(Request $request, HostingAccount $hostingAccount): JsonResponse
    {
        ...
        $this->hostingService->audit(
            $hostingAccount,
            'hosting.updated',
            "Hosting account {$hostingAccount->username} updated via API",
            ['by' => $request->user()?->email],
        );
    ...
    private function rules(?HostingAccount $account = null, int $customerId = 0, bool $partial = false): array
    {
        ...
        $domainRules = ['nullable', 'string', 'max:255', Rule::regex('/^[a-zA-Z0-9]([a-zA-Z0-9\-]*[a-zA-Z0-9])?(\.[a-zA-Z]{2,})+$/')];
