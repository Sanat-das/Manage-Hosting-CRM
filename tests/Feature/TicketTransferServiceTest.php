...
class TicketTransferServiceTest extends TestCase
{
    ...
    /**
     * Moving a ticket between desks says nothing about who it is waiting on,
     * so the status survives the transfer — the WHMCS behaviour. This used to
     * force 'open', which threw away the fact that a ticket was in
     * 'customer_reply' (waiting on staff) or 'on_hold' and made it arrive at
     * the new department looking like nobody was waiting on it.
     */
    public function test_preserves_status_when_transferring(): void
    {
        $actor = User::factory()->create(['role' => 'admin']);

        foreach (['answered', 'customer_reply', 'on_hold', 'in_progress', 'open'] as $status) {
            $ticket = $this->makeTicket('support', $status);

            $result = app(TicketService::class)->transferDepartment($ticket, 'billing', $actor);

            $this->assertSame($status, $result->status, "transfer changed status from {$status}");
            $this->assertSame($status, $result->fresh()->status, "transfer did not persist status {$status}");
        }
    }
