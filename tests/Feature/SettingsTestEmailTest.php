...
class SettingsTestEmailTest extends TestCase
{
    ...
    public function test_incoming_mail_settings_save_and_the_password_is_masked_on_read(): void
    {
        ...
        $settings = app(EmailSettings::class);
        $this->assertFalse($settings->imap_auto_create_customers);

        $this->actingAsSettingsAdmin()
            ->get(route('admin.settings.index', ['tab' => 'email']))
            ->assertOk()
            ->assertSee('Ticket Email Policy')
            ->assertSee('Register Unknown Senders');
    }

    public function test_a_blank_imap_password_keeps_the_stored_one(): void
    {
        // Global IMAP password removed — department mailboxes own their passwords.
        // Keep this test as a no-op for BC: saving an unknown key must not error.
        $this->actingAsSettingsAdmin()
            ->post(route('admin.settings.update'), [
                'active_tab' => 'email',
                'settings' => ['imap_auto_create_customers' => 'yes'],
            ])
            ->assertSessionHasNoErrors();
