...
class HostingIpCardTest extends TestCase
{
    ...
    {
        // Panel user WITHOUT the hosting.manage permission.
    ...
    public function test_edit_page_contains_ip_assignment_controls(): void
    {
        ...
        // The free pool is browsed through the full-pool JSON search (loaded
        // on page open, extended by typing) — not a server-rendered slice —
        // so the created IP must be discoverable through that endpoint.
        $this->actingAsAdmin()
            ->get(route('admin.hosting.available-ips', $account).'?q=10.1.0')
            ->assertOk()
            ->assertJsonFragment(['ip_address' => '10.1.0.22']);
    }
