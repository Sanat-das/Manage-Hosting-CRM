 ...
 *
 * The jobs and scheduled commands existed but had never been proven to run:
 * SendEmail crashed (missing body/error/updated_at columns), and
 * billing:recurring crashed (hosting_accounts had no next_due_date column).
...
class QueueSchedulerTest extends TestCase
{
    ...
    public function test_generate_invoice_job_creates_sent_invoice(): void
    {
        $customer = $this->makeCustomer();

        GenerateInvoice::dispatchSync($customer->id, 'Hosting renewal: example.com', 1499.00, null);
        ...
        $this->assertDatabaseHas('invoices', [
            ...
            'total' => 1499.00,
            'status' => 'sent',
            'notes' => 'Hosting renewal: example.com',
        ]);
    ...
    public function test_generate_invoice_job_dispatches_on_billing_queue(): void
    {
        Queue::fake();

        GenerateInvoice::dispatch(1, 'desc', 10.00);

        Queue::assertPushedOn('billing', GenerateInvoice::class);
    }
    ...
    public function test_billing_recurring_queues_invoices_for_due_accounts(): void
    {
        Queue::fake();
        ...
        // Account outside the window — must not be billed.
        HostingAccount::create([
            'customer_id' => $customer->id,
            'product_id' => $product->id,
            'username' => 'acc2',
        ...
        $this->artisan('billing:recurring', ['--days' => 7])
            ->expectsOutputToContain('Found 1 hosting accounts due within 7 days.')
            ->assertExitCode(0);

        Queue::assertPushed(GenerateInvoice::class, 1);
        Queue::assertPushed(GenerateInvoice::class, function (GenerateInvoice $job) {
            return str_contains($job->description, 'due.example.com')
                && $job->amount === 499.0;
        });
    }

    public function test_billing_recurring_skips_zero_price_products(): void
    {
        Queue::fake();

        $customer = $this->makeCustomer();
        $product = Product::create(['name' => 'Free Addon', 'price' => 0]);
        ...
        $this->artisan('billing:recurring', ['--days' => 7])->assertExitCode(0);

        Queue::assertNotPushed(GenerateInvoice::class);
    }
