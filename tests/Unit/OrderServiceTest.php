...
class OrderServiceTest extends TestCase
{
    ...
    public function test_failed_to_active_retry_reprovisions_and_seeds_billing(): void
    {
        $order = $this->makeOrder();
        $order->product->update(['require_public_ip' => true]);

        // Drive the order into the state an auto-provisioning failure leaves
        // it in (paid -> provisioning -> failed).
        $this->service->markPaid($order);
        $this->service->markProvisioning($order->fresh());
        $this->service->fail($order->fresh());
        ...
        // The manual retry (failed -> active) must re-run what the rolled-back
        // activation skipped: billing seeding + hosting account provisioning.
        $this->service->activate($order->fresh());
        ...
        $history = OrderStatusHistory::where('order_id', $order->id)
            ->orderBy('id')
            ->get();
        $this->assertSame(['failed', 'active'], [$history->last()->from_status, $history->last()->to_status]);
    }

    public function test_activation_provisions_ip_less_hosting_product(): void
    {
        $order = $this->makeOrder();
        ...
        // A hosting-group product WITHOUT IP flags must still get a hosting
        // account (the service row) — IP leasing is best-effort on top.
        $account = HostingAccount::where('order_id', $order->id)->sole();
        $this->assertSame(HostingService::STATUS_PENDING, $account->status);
    }
