...
@section('content')
    ...
    <x-adminlte.partials.datatable icon="bi bi-arrow-left-right" title="All Transactions"
        ...
        :columns="[
            ['label' => '#'], ['label' => 'Customer'], ['label' => 'Invoice'],
            ['label' => 'Amount', 'class' => 'text-end'], ['label' => 'Fee', 'class' => 'text-end'],
            ['label' => 'Net', 'class' => 'text-end'], ['label' => 'Method'],
            ['label' => 'Status'], ['label' => 'Date'],
        ]" :pagination="$transactions">
        @forelse ($transactions as $tx)
            <tr>
                <td><a href="{{ route('admin.transactions.show', $tx) }}">{{ $tx->id }}</a></td>
                <td>{{ $tx->customer?->full_name ?? '—' }}</td>
                <td>{{ $tx->invoice?->invoice_no ?? '—' }}</td>
