        ...
        <script>
            ...
            (function () {
                ...
                function promoBadge(data) {
                    ...
                    return active ? '<span class="badge text-bg-warning">Promo pricing active</span> ' : '';
                }
                ...
                function infoPanel(data) {
                    ...
                    badges.push('<span class="badge text-bg-secondary">' + (paymentLabels[data.paymentType] ?? data.paymentType) + '</span>');

                    const gst = data.gst ?? {};
                    if (gst.enabled) {
                        const rate = (gst.cgst ?? 0) + (gst.sgst ?? 0) || (gst.igst ?? 0);
                        badges.push('<span class="badge text-bg-info">GST ' + rate + '%</span>');
                    }

                    const at = data.autoTerminate ?? {};
                    if (Number(at.value) > 0) {
                        badges.push('<span class="badge text-bg-light border">Auto-terminates after ' + at.value + ' ' + at.unit + '</span>');
                    }

                    if (data.singleUnit) badges.push('<span class="badge text-bg-light border">Single unit</span>');
