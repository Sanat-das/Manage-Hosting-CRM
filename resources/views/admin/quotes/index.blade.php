...
@section('content')
    ...
    <x-adminlte.partials.metric-cards :items="[
        ...
        ['title' => $stats->get('expired', 0), 'text' => 'Expired', 'icon' => 'bi bi-clock-history', 'theme' => 'warning'],
    ]" />
    ...
    <x-adminlte.partials.datatable icon="bi bi-file-text" title="All Quotes"
        ...
        :columns="[
            ['label' => 'Quote #'], ['label' => 'Customer'], ['label' => 'Subject'],
            ['label' => 'Total', 'class' => 'text-end'], ['label' => 'Stage'],
            ['label' => 'Valid until'], ['label' => 'Actions', 'class' => 'text-end'],
        ]" :pagination="$quotes">
        <x-slot name="tools">
            @can('invoices.create')
        ...
        @forelse ($quotes as $quote)
            <tr>
                ...
                <td class="text-end">
                    <a href="{{ route('admin.quotes.show', $quote) }}" class="btn btn-sm btn-outline-secondary"><i class="bi bi-eye"></i></a>
                </td>
            </tr>
        @empty
