...
@section('content')
    ...
    <x-adminlte.partials.form-card
        ...
        title="Edit SSL Certificate"
        :action="route('admin.ssl.update', $ssl)"
        method="PUT"
        submit-label="Save Changes"
        :cancel-url="route('admin.ssl.show', $ssl)"
    >
        <div class="row">
            <div class="col-md-6">
                <x-adminlte-select name="customer_id" label="Customer" required>
                    @foreach ($customers as $id => $name)
                        <option value="{{ $id }}" @selected((int) old('customer_id', $ssl->customer_id) === $id)>{{ $name }}</option>
                    @endforeach
                </x-adminlte-select>
            </div>
            <div class="col-md-6">
                <x-adminlte-input name="domain_name" label="Domain name" placeholder="example.com"
                                  value="{{ old('domain_name', $ssl->domain_name) }}" required />
            </div>
        </div>
            ...
            <div class="col-md-6">
                <x-adminlte-select name="certificate_type" label="Certificate type" required>
                    @foreach (['single' => 'Single', 'wildcard' => 'Wildcard', 'multidomain' => 'Multi-Domain'] as $value => $label)
                        <option value="{{ $value }}" @selected(old('certificate_type', $ssl->certificate_type) === $value)>{{ $label }}</option>
                    @endforeach
                </x-adminlte-select>
            </div>
            <div class="col-md-6">
                <x-adminlte-input name="provider" label="Provider" placeholder="e.g. Let's Encrypt, Sectigo"
                                  value="{{ old('provider', $ssl->provider) }}" />
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-6">
                <x-adminlte-input name="issue_date" type="date" label="Issue date"
                                  value="{{ old('issue_date', $ssl->issue_date?->format('Y-m-d')) }}" />
            </div>
            <div class="col-md-6">
                <x-adminlte-input name="expiry_date" type="date" label="Expiry date"
                                  value="{{ old('expiry_date', $ssl->expiry_date?->format('Y-m-d')) }}" />
            </div>
        </div>
            ...
            <div class="col-md-6">
                <x-adminlte-select name="status" label="Status">
                    @foreach (['active' => 'Active', 'pending' => 'Pending', 'expired' => 'Expired', 'revoked' => 'Revoked', 'failed' => 'Failed'] as $value => $label)
                        <option value="{{ $value }}" @selected(old('status', $ssl->status) === $value)>{{ $label }}</option>
                    @endforeach
                </x-adminlte-select>
            </div>
            <div class="col-md-6">
                <x-adminlte-input name="order_id" type="number" label="Order ID (optional)" min="1"
                                  value="{{ old('order_id', $ssl->order_id) }}" />
            </div>
        </div>

        <x-adminlte-textarea name="notes" label="Notes" rows="3"
                             placeholder="Internal notes (optional)">{{ old('notes', $ssl->notes) }}</x-adminlte-textarea>
    </x-adminlte.partials.form-card>
@stop
