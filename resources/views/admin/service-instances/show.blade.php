        ...
        <div class="col-md-4">
            ...
            <x-adminlte-card icon="bi bi-exclamation-triangle" title="Actions" class="mt-3">
                @if ($serviceInstance->status === 'active')
                    <form method="POST" action="{{ route('admin.service-instances.suspend', $serviceInstance) }}" onsubmit="return confirm('Suspend this service?')">
                        @csrf <button type="submit" class="btn btn-warning w-100 mb-2"><i class="bi bi-pause-circle me-1"></i> Suspend</button>
                    </form>
                @endif
                @if ($serviceInstance->status !== 'terminated')
                    <form method="POST" action="{{ route('admin.service-instances.terminate', $serviceInstance) }}" onsubmit="return confirm('Terminate this service? This cannot be undone.')">
                        @csrf <button type="submit" class="btn btn-danger w-100"><i class="bi bi-x-circle me-1"></i> Terminate</button>
                    </form>
                @endif
            </x-adminlte-card>
        </div>
