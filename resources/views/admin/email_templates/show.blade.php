        ...
        <div class="col-lg-6">
            <x-adminlte-card icon="bi bi-info-circle" title="Details">
                <table class="table table-sm table-borderless">
                    ...
                    <tr><th class="text-muted">Type</th><td class="text-capitalize">{{ $template->type ?? '—' }}</td></tr>
                    <tr><th class="text-muted">Status</th><td>{{ $template->is_active ? '✅ Active' : '⏸️ Inactive' }}</td></tr>
                </table>
            </x-adminlte-card>
        </div>
        <div class="col-lg-6">
            <x-adminlte-card icon="bi bi-code-slash" title="Body Preview">
                <div style="white-space: pre-wrap; background: #f8f9fa; padding: 1rem; border-radius: 0.5rem; max-height: 400px; overflow-y: auto;">{{ $template->body }}</div>
            </x-adminlte-card>
        </div>
    </div>
@stop
