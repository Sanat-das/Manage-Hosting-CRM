...
@php
    ...
    $tabs = [
        ...
        ['id' => 'hosting', 'label' => 'Hosting', 'icon' => 'bi bi-hdd-stack'],
                    ...
                    <div class="border-bottom py-2">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                @if ($account->domain)
                                    <span class="text-muted">— {{ $account->domain }}</span>
                                @endif
                            </div>
                            <x-adminlte.partials.status-badge :status="$account->status" />
                        </div>
                        <div class="text-muted small mt-1">
                            Disk: {{ $account->disk_used }} / {{ $account->disk_quota }} MB
                            &middot; Bandwidth: {{ $account->bandwidth_used }} / {{ $account->bandwidth_quota }} MB
                        </div>
                    </div>
                @empty
                    <p class="text-muted mb-0">No hosting accounts yet.</p>
                @endforelse
            </div>
                        ...
                        <tbody>
                            @forelse ($customer->domains as $domain)
                                <tr>
                                    ...
                                    <td class="text-muted">{{ $domain->expiry_date?->format('M j, Y') ?? '—' }}</td>
            ...
            <div class="tab-pane fade {{ $activeTab === 'contacts' ? 'show active' : '' }}" id="contacts" role="tabpanel" aria-labelledby="contacts-tab">
                ...
                <div class="table-responsive">
                    <table class="table table-sm align-middle mb-0">
                        <thead>
                            <tr>
                                <th>Name</th><th>Email</th><th>Phone</th><th>Role</th><th>Type</th>
                            </tr>
                        </thead>
                        <tbody>
