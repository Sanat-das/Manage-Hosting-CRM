...
@section('content')
        ...
        <div class="d-flex flex-wrap align-items-center gap-3">
            ...
            <div class="flex-grow-1">
                <div class="d-flex align-items-center flex-wrap gap-2">
                    ...
                    @if ($customer->company)
                        <span class="badge text-bg-info">{{ $customer->company }}</span>
                    @endif
                </div>
            ...
            <div class="d-flex gap-2">
                ...
                @if (\Illuminate\Support\Facades\Route::has('admin.impersonate.start'))
                    <button type="button" class="btn btn-sm btn-outline-secondary" title="Log in as this customer"
                            data-bs-toggle="modal" data-bs-target="#impersonate-customer-modal">
                    ...
                    <table class="table table-sm align-middle mb-0">
                        ...
                        <tbody>
                            @forelse ($customer->domains as $domain)
                                ...
                                <tr>
                                    <td><a href="{{ route('admin.domains.show', $domain) }}">{{ $domain->name }}</a></td>
    ...
    @can('customers.edit')
        @foreach ($customer->contacts as $contact)
            <x-adminlte.partials.confirm-modal
                ...
                :message="'Delete contact ' . ($contact->name ?? '#' . $contact->id) . '? This cannot be undone.'"
                :action="route('admin.customers.contacts.destroy', [$customer, $contact])"
    ...
    @if (\Illuminate\Support\Facades\Route::has('admin.impersonate.start'))
        <x-adminlte.partials.confirm-modal
            id="impersonate-customer-modal"
            title="Log in as customer"
            :message="'Log in as ' . $customer->full_name . '? You will be signed in as this customer until you stop impersonating.'"
            method="POST"
            :action="route('admin.impersonate.start', $customer->user_id)"
            confirm-label="Log in as customer"
