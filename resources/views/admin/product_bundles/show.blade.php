...
@section('content_header')
    <x-ui.page-header title="{{ $bundle->name }}" subtitle="View product bundle details and history" :breadcrumbs="[
        ['label' => __('adminlte.home'), 'url' => url('/')],
        ['label' => 'Product Bundles', 'url' => route('admin.product-bundles.index')],
        ['label' => $bundle->name, 'active' => true],
    ]" />
@stop
@section('content')
    ...
    <x-adminlte-card icon="bi bi-info-circle" title="Details">
        <table class="table table-sm table-borderless mb-0">
            <tbody>
                ...
                <tr><th class="text-muted">Type</th><td><span class="badge text-bg-info">Bundle</span></td></tr>
                <tr><th class="text-muted">Status</th><td><x-adminlte.partials.status-badge :status="$bundle->status" /></td></tr>
            </tbody>
        </table>
    </x-adminlte-card>
                ...
                <tbody>
                    @forelse ($bundle->bundleChildren as $i => $row)
                        <tr>
                            ...
                            <td>{{ $row->quantity }}</td>
                            <td><span class="badge text-bg-secondary">{{ ucfirst($row->discount_type) }}</span></td>
                            <td class="text-end">
