...
@section('content_header')
    <div class="row">
        <div class="col-sm-6">
            <h1 class="m-0">Edit Server Group</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-end">
                <li class="breadcrumb-item"><a href="{{ url('/') }}">{{ __('adminlte.home') }}</a></li>
                <li class="breadcrumb-item"><a href="{{ route('admin.server-groups.index') }}">Server Groups</a></li>
                <li class="breadcrumb-item active" aria-current="page">{{ $serverGroup->name }}</li>
            </ol>
        </div>
    </div>
@stop
            ...
            @forelse ($servers as $server)
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="server_ids[]"
                           ...
                           @checked(in_array($server->id, old('server_ids', $selectedServerIds), true))>
