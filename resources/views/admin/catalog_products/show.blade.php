...
@section('content_header')
    <x-ui.page-header title="{{ $product->name }}" subtitle="View catalog product details and history" :breadcrumbs="[
            ...
            <x-adminlte-card icon="bi bi-info-circle" title="Details">
                <table class="table table-sm table-borderless mb-0">
                    <tbody>
                        ...
                        <tr><th class="text-muted">Type</th><td><span class="badge text-bg-info">{{ ucfirst($product->product_type) }}</span></td></tr>
        ...
        <div class="col-md-4">
            <x-adminlte-card icon="bi bi-gear" title="Options">
                @if ($product->require_domain)
                    <span class="badge text-bg-warning me-1">Requires Domain</span>
                @endif
                @if ($product->show_in_order)
                    <span class="badge text-bg-success me-1">Visible in Order</span>
                @endif
