...
@php
    ...
    if ($errors->any()) {
        ...
        if ($errorKeys->contains(fn ($key) => str_starts_with((string) $key, 'option_links') || str_starts_with((string) $key, 'option_groups'))) {
            ...
            $activeTab = 'pricing';
        } elseif ($errorKeys->contains(fn ($key) => str_starts_with((string) $key, 'config.'))) {
            $activeTab = 'modules';
        }
    }
@endphp
...
@section('content_header')
    <x-ui.page-header :title="'Edit ' . $product->name" subtitle="Update product details, pricing and configuration" :breadcrumbs="[
...
@section('content')
    ...
    >
        ...
        <ul class="nav nav-tabs mb-3" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                <button class="nav-link {{ $activeTab === 'modules' ? 'active' : '' }}" id="edit-tab-modules"
            ...
            <div class="tab-pane fade {{ $activeTab === 'modules' ? 'show active' : '' }}" id="edit-pane-modules"
                ...
                @forelse ($modules as $module)
                    ...
                    <div class="border rounded p-3 mb-3">
                        ...
                        @if ($link?->enabled && ! empty($schema['fields']))
    ...
    @foreach ($product->optionLinks as $link)
        @if ($link->group && ! in_array($link->group->type, \App\Models\ProductOptionGroup::CONTINUOUS_TYPES, true))
            <x-adminlte.partials.confirm-modal
                ...
                title="Sync values from group"
                :message="'Replace this product\'s values with the current values from ' . ($link->group?->name ?? 'the option group') . '? Per-product tweaks will be overwritten.'"
                method="POST"
                :action="route('admin.products.options.sync', [$product, $link])"
                confirm-label="Sync values"
    ...
    @push('js')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                // option attach picker). redirect:'manual' keeps the
                // controller's flash / validation errors in the session so
                ...
                document.querySelectorAll('[data-module-toggle]').forEach(function (btn) {
                    btn.addEventListener('click', function () {
                        moduleAction(btn.dataset.url, 'POST', new FormData());
                    });
                });

                document.querySelectorAll('.module-config-save').forEach(function (btn) {
                    btn.addEventListener('click', function () {
                        var data = new FormData();
                        var container = btn.closest('.border');
                        container.querySelectorAll('.module-config-fields input, .module-config-fields select, .module-config-fields textarea').forEach(function (field) {
