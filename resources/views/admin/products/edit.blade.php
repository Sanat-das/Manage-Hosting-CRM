...
@section('content')
    ...
    <x-adminlte.partials.form-card
        ...
        :cancel-url="route('admin.products.show', $product)"
    >
        <div class="row">
            <div class="col-md-6">
        ...
        <x-adminlte-card title="GST settings" icon="bi bi-percent" class="mt-3" body-class="p-3">
            <div class="form-check mb-3">
                <input class="form-check-input" type="checkbox" name="gst_enabled" value="1"
                       id="gst_enabled" @checked(old('gst_enabled', $product->gst_enabled))>
                ...
                <div class="col-md-4">
                    <x-adminlte-select name="gst_type" label="GST type">
                        @foreach ($gstTypes as $value => $label)
                            <option value="{{ $value }}" @selected(old('gst_type', $product->gst_type) === $value)>{{ $label }}</option>
                        @endforeach
                    </x-adminlte-select>
                </div>
                <div class="col-md-4">
                ...
                <table class="table table-sm align-middle mb-0">
                    ...
                    <tbody>
                        @foreach ($cycles as $cycle => $cycleLabel)
                            @php $pricingRow = $product->pricing->firstWhere('billing_cycle', $cycle); @endphp
                            <tr>
                                <td><strong>{{ $cycleLabel }}</strong></td>
                                <td>
                                    <input type="number" name="pricing[{{ $cycle }}][price]" step="0.01" min="0"
                                           class="form-control form-control-sm text-end"
                                           value="{{ old("pricing.$cycle.price", $pricingRow?->price) }}" placeholder="0.00">
                                </td>
                                <td>
                                    <input type="number" name="pricing[{{ $cycle }}][setup_fee]" step="0.01" min="0"
                                           class="form-control form-control-sm text-end"
                                           value="{{ old("pricing.$cycle.setup_fee", $pricingRow?->setup_fee) }}" placeholder="0.00">
                                </td>
                                <td>
                                    <input type="number" name="pricing[{{ $cycle }}][promo_price]" step="0.01" min="0"
                                           class="form-control form-control-sm text-end"
                                           value="{{ old("pricing.$cycle.promo_price", $pricingRow?->promo_price) }}" placeholder="0.00">
                                </td>
                                <td>
                                    <input type="date" name="pricing[{{ $cycle }}][promo_start]"
                                           class="form-control form-control-sm"
                                           value="{{ old("pricing.$cycle.promo_start", $pricingRow?->promo_start?->format('Y-m-d')) }}">
                                </td>
                                <td>
                                    <input type="date" name="pricing[{{ $cycle }}][promo_end]"
                                           class="form-control form-control-sm"
                                           value="{{ old("pricing.$cycle.promo_end", $pricingRow?->promo_end?->format('Y-m-d')) }}">
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
