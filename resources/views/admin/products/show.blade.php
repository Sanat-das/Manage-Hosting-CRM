        ...
        <div class="d-flex flex-wrap align-items-center gap-3">
            ...
            <div class="flex-grow-1">
                <div class="d-flex align-items-center flex-wrap gap-2">
                    ...
                    <span class="badge bg-info">{{ $typeLabels[$product->type] ?? ucfirst($product->type) }}</span>
    ...
    <x-adminlte-card>
        <x-adminlte.partials.detail-tabs :tabs="$tabs" :active-tab="$activeTab">
            ...
            <div class="tab-pane fade {{ $activeTab === 'overview' ? 'show active' : '' }}" id="overview" role="tabpanel" aria-labelledby="overview-tab">
                    ...
                    <div class="col-md-6">
                        <table class="table table-sm table-borderless">
                            <tbody>
                                ...
                                <tr><th class="text-muted">Type</th><td>{{ $typeLabels[$product->type] ?? ucfirst($product->type) }}</td></tr>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <h6 class="text-muted mt-3">Resource quotas</h6>
                        <table class="table table-sm table-borderless">
                            <tbody>
                                <tr><th class="text-muted w-25">Disk (MB)</th><td>{{ $product->quota_disk ?: '—' }}</td></tr>
                                <tr><th class="text-muted">Bandwidth (MB)</th><td>{{ $product->quota_bandwidth ?: '—' }}</td></tr>
                                <tr><th class="text-muted">Email accounts</th><td>{{ $product->quota_email ?: '—' }}</td></tr>
                                <tr><th class="text-muted">Databases</th><td>{{ $product->quota_database ?: '—' }}</td></tr>
                                <tr><th class="text-muted">CPU cores</th><td>{{ $product->quota_cpu_cores ?: '—' }}</td></tr>
                                <tr><th class="text-muted">CPU speed (MHz)</th><td>{{ $product->quota_cpu_speed ?: '—' }}</td></tr>
                                <tr><th class="text-muted">RAM (MB)</th><td>{{ $product->quota_ram ?: '—' }}</td></tr>
                                <tr><th class="text-muted">IPs / FTP / Subdomains</th><td>{{ $product->quota_ips }} / {{ $product->quota_ftp_accounts }} / {{ $product->quota_subdomains }}</td></tr>
                            </tbody>
                        </table>
                    </div>
                    <div class="col-md-6">
            ...
            <div class="tab-pane fade {{ $activeTab === 'options' ? 'show active' : '' }}" id="options" role="tabpanel" aria-labelledby="options-tab">
                @forelse ($product->options as $option)
                    <div class="border rounded p-3 mb-3">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <span class="badge bg-info">{{ ucfirst($option->type) }}</span>
                        </div>
                                    ...
                                    <tbody>
                                        @foreach ($option->values as $value)
                                            <tr>
                                                ...
                                                @foreach ($cycleLabels as $cycle => $label)
                                                    @php
                                                        $modifier = $value->pricing->firstWhere('billing_cycle', $cycle);
                                                    @endphp
                ...
                @empty
                    <p class="text-muted mb-0">No configurable options.</p>
                @endforelse
            </div>
